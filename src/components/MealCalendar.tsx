        ...
        {isManager && allMembers.length > 0 && (
          <div className="pt-3 border-t border-slate-800/80 space-y-1.5">
            <div className="flex items-center justify-between">
              <label htmlFor="calendar-member-select" className="text-xs text-amber-300 font-bold flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-amber-400" />
                <span>মেম্বার নির্বাচন (ক্যালেন্ডার):</span>
              </label>
              {selectedMemberObj && (
                <span className="text-[10px] text-slate-400">
                  নির্বাচিত: <strong className="text-amber-300">{selectedMemberObj.name}</strong>
                </span>
              )}
            </div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full bg-slate-900/95 border border-amber-500/40 hover:border-amber-500/70 focus:border-amber-500 rounded-xl px-3.5 py-2 text-xs font-semibold text-white focus:outline-none transition-all cursor-pointer shadow-inner appearance-none pr-8"
              >
                {allMembers.map((m) => (
                  <option key={m.id} value={m.id} className="bg-slate-900 text-white py-1.5">
                    {m.name} {m.id === currentUser?.id ? '★ (আপনি)' : ''} — {m.role === 'ADMIN' ? 'সুপার এডমিন' : m.role === 'MANAGER' ? 'ম্যানেজার' : 'মেম্বার'}
                  </option>
                ))}
              </select>
      ...
      {activeDateStr && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fade-in">
          <div className="glass-panel w-full max-w-sm rounded-3xl p-5 border border-slate-700 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            {/* Manager: Member Selector Inside Modal */}
            {isManager && allMembers.length > 0 && (
              <div className="mb-3.5 p-3 rounded-2xl bg-slate-900/90 border border-amber-500/30 space-y-1.5 shadow-sm">
                <label className="text-[11px] font-bold text-amber-300 flex items-center gap-1">
                  <UserIcon className="w-3.5 h-3.5 text-amber-400" />
                  <span>কার মিল পরিবর্তন করছেন:</span>
                </label>
                <div className="relative">
                  <select
                    value={selectedUserId}
                    onChange={(e) => {
                      ...
                      if (activeDateStr) {
                        fetchDateAllMeals(activeDateStr, newId);
                      }
                    }}
                    className="w-full bg-slate-950 border border-slate-700 hover:border-amber-500/60 rounded-xl px-3 py-2 text-xs font-semibold text-white focus:outline-none focus:border-amber-500 cursor-pointer shadow-inner appearance-none pr-8"
                  >
                    {allMembers.map((m) => (
                  ...
                  <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2.5 text-amber-400">
