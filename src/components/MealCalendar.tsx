...
export default function MealCalendar({
  ...
  const handleDayClick = (day: number) => {
    ...
    const existingMeal = meals.find((m) => m.date === targetDateStr);
    if (existingMeal) {
      setActiveMeal({
        lunch: existingMeal.lunch,
        dinner: existingMeal.dinner,
        isLunchCooked: Boolean(existingMeal.isLunchCooked),
        isDinnerCooked: Boolean(existingMeal.isDinnerCooked),
        updatedTime: existingMeal.updatedTime,
      });
    } else {
      setActiveMeal({
        lunch: 0,
          ...
          <div className="glass-panel w-full max-w-sm rounded-3xl p-5 border border-slate-700 shadow-2xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-3">
              <span className="text-xs text-emerald-400 font-semibold uppercase tracking-wider">
                {selectedMemberObj?.name || 'মেম্বার'}
              </span>
            ...
            <div className="space-y-3 mb-4">
              ...
              <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/80 border border-slate-800">
                <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-300">
                  ...
                  <span>☀️ দুপুরের মিল</span>
                  {activeMeal.isLunchCooked ? (
                    <span className="text-[9px] bg-emerald-500/20 text-emerald-300 px-1.5 py-0.2 rounded">রান্না শেষ</span>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-slate-800 hover:bg-slate-700 disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center text-slate-200"
                  >
                  ...
                  <button
                    ...
                    className="w-8 h-8 rounded-lg bg-emerald-600 hover:bg-emerald-500 disabled:opacity-30 disabled:cursor-not-allowed flex items-center justify-center text-white"
                  >
            ...
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setActiveDateStr(null)}
                className="w-1/3 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-semibold transition-all"
              >
                বন্ধ করুন
              </button>
              <button
