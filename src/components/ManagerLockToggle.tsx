  ...
  return (
    <div className="glass-card rounded-2xl p-4 border border-amber-500/30 bg-gradient-to-r from-amber-950/20 via-slate-900/80 to-slate-900/90 shadow-lg space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2.5">
        ...
        <div className="bg-slate-900/80 p-3 rounded-2xl border border-slate-800 flex flex-col justify-between space-y-2">
          ...
          <div className="space-y-1.5">
            ...
            <button
              ...
              className={`w-full py-1.5 px-2 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1 transition-all disabled:opacity-40 ${
                lunchLocked
            ...
            {!todayCookedStatus.isLunchCooked ? (
              <button
                ...
                className="w-full py-1.5 px-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-slate-950 font-black text-[11px] flex items-center justify-center gap-1 shadow-md shadow-emerald-500/20 transition-all active:scale-95 disabled:opacity-50"
              >
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>রান্না হয়ে গেছে</span>
              </button>
            ) : (
              <div className="text-[10px] text-center text-emerald-400 font-semibold bg-emerald-500/10 py-1 rounded-xl">
                🔒 মিল ফাইনাল ও গণনাভুক্ত
              </div>
            )}
          </div>
        </div>
      ...
      {message && (
        <div className="text-center text-xs font-semibold text-emerald-300 bg-emerald-500/10 py-1.5 px-2 rounded-xl border border-emerald-500/20">
          {message}
        </div>
      )}
    </div>
